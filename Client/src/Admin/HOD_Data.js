export const HOD_Data = [
  // {
  //   value: "admin",
  //   text: "-- Select Dept --",
  //   hod: ["td@hyderabad.bits-pilani.ac.in"],
  // },
  {
    value: "Biological Sciences",
    text: "Biological Sciences",
    hod: ["hod.biology@hyderabad.bits-pilani.ac.in","noreplytd@hyderabad.bits-pilani.ac.in"],
  },
  {
    value: "Chemical Engineering",
    text: "Chemical Engineering",
    hod: ["hod.chemical@hyderabad.bits-pilani.ac.in"],
  },
  {
    value: "Chemistry",
    text: "Chemistry",
    hod: ["hod.chemistry@hyderabad.bits-pilani.ac.in"],
  },
  {
    value: "Civil Engineering",
    text: "Civil Engineering",
    hod: ["hod.civil@hyderabad.bits-pilani.ac.in"],
  },
  { 
    value: "Computer Science and Information Systems",
    text: "Computer Science and Information Systems",
    hod: ["hod.csis@hyderabad.bits-pilani.ac.in"]
  },
  {
    value: "Economics and Finance",
    text: "Economics and Finance",
    hod: ["hod.ecofin@pilani.bits-pilani.ac.in"],
  },
  { 
    value: "Electrical & Electronics Engineering",
    text: "Electrical & Electronics Engineering",
    hod: ["hod.eee@hyderabad.bits-pilani.ac.in"]
  },
  {
    value: "Humanities and Social Sciences",
    text: "Humanities and Social Sciences",
    hod: ["hod.hss@hyderabad.bits-pilani.ac.in"],
  },
  {
    value: "Mathematics",
    text: "Mathematics",
    hod: ["hod.maths@hyderabad.bits-pilani.ac.in"],
  },
  {
    value: "Mechanical Engineering",
    text: "Mechanical Engineering",
    hod: ["hod.mechanical@hyderabad.bits-pilani.ac.in"],
  },
  {
    value: "Pharmacy",
    text: "Pharmacy",
    hod: ["hod.pharmacy@hyderabad.bits-pilani.ac.in"],
  },
  {
    value: "Physics",
    text: "Physics",
    hod: ["hod.physics@hyderabad.bits-pilani.ac.in"],
  },
];
