export const HOD_Data = [
  // {
  //   value: "admin",
  //   text: "-- Select Dept --",
  //   hod: ["f20212587@hyderabad.bits-pilani.ac.in"],
  // },
  {
    value: "Biological Sciences",
    text: "Biological Sciences",
    hod: ["gupta.nishith@hyderabad.bits-pilani.ac.in","f20212587@hyderabad.bits-pilani.ac.in", "timetable@hyderabad.bits-pilani.ac.in"],
  },
  {
    value: "Chemical Engineering",
    text: "Chemical Engineering",
    hod: ["sreedhar.p@hyderabad.bits-pilani.ac.in"],
  },
  {
    value: "Chemistry",
    text: "Chemistry",
    hod: ["sounak.roy@hyderabad.bits-pilani.ac.in"],
  },
  {
    value: "Civil Engineering",
    text: "Civil Engineering",
    hod: ["murari@hyderabad.bits-pilani.ac.in"],
  },
  { 
    value: "Computer Science and Information Systems",
    text: "Computer Science and Information Systems",
    hod: ["bhanu@hyderabad.bits-pilani.ac.in"]
  },
  {
    value: "Economics and Finance",
    text: "Economics and Finance",
    hod: ["dushyant@hyderabad.bits-pilani.ac.in"],
  },
  { 
    value: "Electrical & Electronics Engineering",
    text: "Electrical & Electronics Engineering",
    hod: ["sahoo@hyderabad.bits-pilani.ac.in"]
  },
  {
    value: "Humanities and Social Sciences",
    text: "Humanities and Social Sciences",
    hod: ["shilpaa.anand@hyderabad.bits-pilani.ac.in"],
  },
  {
    value: "Mathematics",
    text: "Mathematics",
    hod: ["pksahoo@hyderabad.bits-pilani.ac.in"],
  },
  {
    value: "Mechanical Engineering",
    text: "Mechanical Engineering",
    hod: ["nskreddy@hyderabad.bits-pilani.ac.in"],
  },
  {
    value: "Pharmacy",
    text: "Pharmacy",
    hod: ["sajeli@hyderabad.bits-pilani.ac.in"],
  },
  {
    value: "Physics",
    text: "Physics",
    hod: ["sarmistha.banik@hyderabad.bits-pilani.ac.in"],
  },
];
